...
A read-only client for Pequerecetas, with no API key and no account. It reads,
...
──────────────────────────────────────────────────────────  the seam
src/pequerecetas/*.ts  http, pacing, storage, reading     ← never imports the SDK
```
...
`src/pequerecetas/` file that reaches for the SDK closes that door.
...
Pequerecetas is free to read and publishes no crawl delay, which is a reason to
