...
function client(fetchImpl: typeof fetch): PequerecetasClient {
  return new PequerecetasClient({
    config: loadConfig({ PQR_MAX_BODY_BYTES: String(CAP), PQR_MIN_INTERVAL_MS: "0" }),
...
describe("a page larger than this reader holds", () => {
  it("is refused for its size", async () => {
    ...
    expect(await said(reading.getRecipe("crepes"))).toContain(String(CAP));
  });
  ...
  it("is read when it fits, whatever the page then turns out to hold", async () => {
    ...
    expect(await said(reading.getRecipe("crepes"))).not.toContain(String(CAP));
  });
});
