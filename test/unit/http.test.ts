...
import { fetchPage, parseRetryAfter } from "../../src/pequerecetas/http.js";
import { RateLimiter } from "../../src/pequerecetas/rateLimiter.js";
...
async function failure(promise: Promise<unknown>): Promise<PequerecetasError> {
  const result = await settle(promise);
  if (result.ok) {
    throw new Error(`expected a rejection, resolved with ${String(result.value)}`);
  }
  expect(result.error).toBeInstanceOf(PequerecetasError);
  return result.error as PequerecetasError;
}
...
describe("fetchPage", () => {
  function options(
    ...
    return {
      url: "https://www.pequerecetas.com/search",
  ...
  it("sends the user agent it was handed, asks for JSON and follows redirects", async () => {
    ...
    expect(call.url).toBe("https://www.pequerecetas.com/search");
  ...
  it("turns a thrown transport failure into a PequerecetasError", async () => {
    const rec = recorder([() => Promise.reject(new TypeError("socket closed"))]);
    const error = await failure(fetchPage(options(rec.impl, { maxRetries: 0 })));

    expect(error).toBeInstanceOf(PequerecetasError);
  });

  it("turns a transport rejection that is not an Error into a PequerecetasError", async () => {
  ...
  it("turns a body that cannot be read into a PequerecetasError", async () => {
...
describe("the fetch a caller does not hand over", () => {
  it("is the runtime's own, so a plain call still reaches the site", async () => {
    ...
    const page = await fetchPage({
      url: "https://www.pequerecetas.com/recettes",
      userAgent: "test-agent/1.0",
      timeoutMs: 20_000,
      maxRetries: 0,
      limiter: new RateLimiter({ intervalMs: 1000 }),
...
describe("a refusal carrying no body", () => {
  it("is read like any other, with nothing to let go of", async () => {
    ...
    const error = await failure(
      fetchPage({
        url: "https://www.pequerecetas.com/recettes/cat/absente",
        userAgent: "test-agent/1.0",
        timeoutMs: 20_000,
        maxRetries: 0,
        limiter: new RateLimiter({ intervalMs: 1000 }),
...
describe("the address a page was served from", () => {
  const asked = "https://www.pequerecetas.com/recettes/cat/absente";
  ...
  it("is the one the answer came back from, when a redirect moved it", async () => {
    ...
    Object.defineProperty(moved, "url", { value: "https://www.pequerecetas.com/recettes" });

    expect((await success(pageFrom(moved))).url).toBe("https://www.pequerecetas.com/recettes");
  });
