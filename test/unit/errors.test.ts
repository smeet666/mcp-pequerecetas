...
describe("PequerecetasError", () => {
  it("is an Error named PequerecetasError", () => {
    const error = new PequerecetasError("not_found", "gone");
    expect(error).toBeInstanceOf(Error);
    expect(error).toBeInstanceOf(PequerecetasError);
    expect(error.name).toBe("PequerecetasError");
    expect(error.message).toBe("gone");
    expect(error.code).toBe("not_found");
  });

  it("keeps the details it is given", () => {
    const error = new PequerecetasError("network_error", "boom", {
  ...
  it("attaches a supplied cause to the error itself", () => {
    ...
    const error = new PequerecetasError("network_error", "boom", { cause: root });
...
describe("factories", () => {
  ...
  it("each factory keeps the message and the PequerecetasError shape", () => {
    for (const error of [
      notFound("m1"),
      invalidInput("m2"),
      rateLimited("m3"),
