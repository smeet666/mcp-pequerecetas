...
const numericCases: readonly NumericCase[] = [
  {
    variable: "PQR_MIN_INTERVAL_MS",
    field: "minIntervalMs",
    fallback: 3000,
    min: 3000,
    max: 60_000,
  },
  { variable: "PQR_TIMEOUT_MS", field: "timeoutMs", fallback: 20_000, min: 1000, max: 120_000 },
  { variable: "PQR_MAX_RETRIES", field: "maxRetries", fallback: 3, min: 0, max: 8 },
  { variable: "PQR_CACHE_TTL_MS", field: "cacheTtlMs", fallback: 900_000, min: 0, max: 86_400_000 },
  {
    variable: "PQR_CACHE_MAX_ENTRIES",
...
describe("PQR_LOG_LEVEL", () => {
  it.each(LOG_LEVELS)("accepts %s", (level) => {
    const config = loadConfig({ PQR_LOG_LEVEL: level });
  ...
  it("warns on stderr and keeps error for an unknown level", () => {
    const config = loadConfig({ PQR_LOG_LEVEL: "chatty" });
    expect(config.logLevel).toBe("error");
    expect(stderrLines.length).toBeGreaterThan(0);
    expectSilentStdout();
  });
  ...
  it("keeps error, in silence, when the level is empty", () => {
    expect(loadConfig({ PQR_LOG_LEVEL: "" }).logLevel).toBe("error");
    expect(stderrLines).toEqual([]);
    expectSilentStdout();
  });
...
describe("user agent", () => {
  it("names the package, its version and the repository", () => {
    expect(DEFAULT_USER_AGENT).toContain("mcp-pequerecetas");
    expect(DEFAULT_USER_AGENT).toMatch(/\d+\.\d+\.\d+/);
    expect(DEFAULT_USER_AGENT).toContain("github.com/smeet666/mcp-pequerecetas");
  });

  it("appends the default to a supplied user agent", () => {
    const config = loadConfig({ PQR_USER_AGENT: "my-client/2.0" });
  ...
  it("falls back to the default when the variable is absent, empty or blank", () => {
    ...
    expect(loadConfig({ PQR_USER_AGENT: "" }).userAgent).toBe(DEFAULT_USER_AGENT);
    expect(loadConfig({ PQR_USER_AGENT: "   " }).userAgent).toBe(DEFAULT_USER_AGENT);
    expectSilentStdout();
  });
});
...
describe("createLogger", () => {
  ...
  function messages(): string[] {
    return stderrLines.map((line) => line.replace(/^\[mcp-pequerecetas] \w+: /, "").trimEnd());
  }
  ...
  it("prefixes each line with the server name and the level, and ends it with a break", () => {
    callEveryMethod("debug");
    expect(stderrLines).toEqual([
      "[mcp-pequerecetas] debug: d\n",
      "[mcp-pequerecetas] info: i\n",
      "[mcp-pequerecetas] warn: w\n",
      "[mcp-pequerecetas] error: e\n",
    ]);
    expectSilentStdout();
  });
  ...
  it("writes one line per call, never more", () => {
    ...
    for (const line of stderrLines) {
      ...
      expect(line.startsWith("[mcp-pequerecetas] ")).toBe(true);
    }
  });
