...
async function readBounded(response: Response, maxBytes: number, url: string): Promise<string> {
  ...
  try {
    for (;;) {
      ...
      if (held > maxBytes) {
        throw parseFailure(
          `Pequerecetas answered with more than ${maxBytes} bytes, past what this reads for one page.`,
          { url },
        );
      }
...
async function readRefusal(
  ...
  if (PUSH_BACK.has(response.status)) {
    ...
    if (asked !== null && asked > LONGEST_WAIT_MS) {
      return {
        ...
        pushBack: true,
        error: rateLimited(
          `Pequerecetas asked this client to wait ${Math.round(asked / 1000)} seconds (HTTP ${response.status}).`,
    ...
    if (attempt >= maxRetries) {
      return {
        ...
        pushBack: true,
        error: rateLimited(
          `Pequerecetas asked this client to slow down (HTTP ${response.status}).`,
          {
            url,
  ...
  if (response.status === 400 || response.status === 422) {
    ...
    return {
      ...
      pushBack: false,
      error: invalidInput(
        "Pequerecetas would not accept this request.",
  ...
  if (response.status === 404 || response.status === 410) {
    ...
    return {
      kind: "refused",
      pushBack: false,
      error: notFound("Pequerecetas holds nothing at this address.", {
        url,
        status: response.status,
      }),
    };
  ...
  return {
    kind: "refused",
    pushBack: false,
    error: networkError(`Pequerecetas answered HTTP ${response.status}.`, {
      url,
      status: response.status,
    }),
  };
...
function readFailure(
  ...
  if (error instanceof Error && error.name === "PequerecetasError") {
    throw error;
  }

  if (error instanceof Error && error.name === "AbortError") {
    if (attempt >= Math.min(maxRetries, RETRIES_AFTER_SILENCE)) {
      throw timeoutError(`No answer from Pequerecetas within ${timeoutMs}ms.`, { url });
    }
    return error;
  }
  ...
  if (attempt >= maxRetries) {
    throw networkError(`Could not reach Pequerecetas: ${failure.message}`, { url });
  }
  return failure;
}
...
export async function fetchPage(options: FetchOptions): Promise<Page> {
  ...
  /* v8 ignore next 4 -- The loop leaves by returning or by raising, and its last
     ...
     reaches. */
  throw networkError(`Could not reach Pequerecetas: ${lastError?.message ?? "unknown"}`, { url });
}
