 ...
 * where the line wrote none: "una cebolla" names a vegetable and no amount, and
...
const RANGE_JOINER = /^\s+(a|o)\s+/i;
const LEADING_DASH = /^\s*(–|—|-)\s*/;
const OPENING_ARTICLE = /^\s*(un|una|unos|unas)\b\s*/i;
/** "de", the articles it takes, and the two forms Spanish contracts it into. */
const LEADING_PARTITIVE = /^(?:del\s+|de\s+la\s+|de\s+las\s+|de\s+los\s+|de\s+el\s+|de\s+)/i;
const MEASURE_PARTITIVE = /^(?:del\s+|de\s+la\s+|de\s+las\s+|de\s+los\s+|de\s+el\s+|de\s+)/i;
 ...
 *
 * "unos" and "unas" are not a number the page wrote, and they are not nothing
 * either: a cook reading them takes a few. Three is what a few comes to, and
 * the answer says so rather than letting the reading pass unremarked.
 */
const ARTICLE_AMOUNTS: Record<string, number> = { un: 1, una: 1, unos: 3, unas: 3 };
 ...
 * bare glyph; and a decimal, which Spanish writes with a comma.
 *
 * Returns null when the line does not start with a number, which is the normal
 * case for "sal" or "pimienta recién molida".
 */
...
/**
 * Read a leading range such as "2 a 3", "3-4" or "225–500".
 *
...
export function parseLeadingRange(text: string): ParsedRange | null {
  ...
  // A written separator needs whitespace around it, so "5 cebollas" is not read
...
/** At most this many words make up a measure, which is what "cucharadita de postre" takes. */
 ...
 * many there will be.
 *
 * The longest candidate is tried first, so "cucharada sopera" wins over a
 * shorter word inside it. A measure may stand behind the word introducing it,
 * as in "3/4 de taza", and the two spellings name one quantity.
 */
 ...
 * is where it stands for a number: "una pizca" is one pinch, while "una cebolla"
...
export interface FormatAmountOptions {
   ...
   *
   * True for things a cook counts or spoons out: "1/3 taza" is how a kitchen
   * expresses it, while "0,33 taza" is not. False for mass and volume, which are
...
/** Render an amount the way a Spanish recipe writes it, comma and all. */
