...
/**
 * Pequerecetas publishes no crawl delay in its robots.txt, so this floor is
...
export const DEFAULT_USER_AGENT = `mcp-pequerecetas/${PKG_VERSION} (+${REPO_URL})`;
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  const level = env.PQR_LOG_LEVEL?.trim();
  ...
  if (named && !LOG_LEVELS.includes(named)) {
    process.stderr.write(
      `${PREFIX} error: PQR_LOG_LEVEL="${named}" is not one of ${LOG_LEVELS.join(", ")}; using error.\n`,
    );
  }

  const custom = env.PQR_USER_AGENT?.trim();
  ...
  return {
    ...
    timeoutMs: readInteger(env, "PQR_TIMEOUT_MS", 20_000, 1000, 120_000),
    maxRetries: readInteger(env, "PQR_MAX_RETRIES", 3, 0, 8),
    maxBodyBytes: readInteger(env, "PQR_MAX_BODY_BYTES", 8_000_000, 100_000, 64_000_000),
    ...
    cacheTtlMs: readInteger(env, "PQR_CACHE_TTL_MS", 900_000, 0, 86_400_000),
    cacheMaxEntries: readInteger(env, "PQR_CACHE_MAX_ENTRIES", 200, 1, 5000),
    logLevel,
  };
}
