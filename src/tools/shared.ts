...
export const SOURCE_NAME = "Pequerecetas";
export const ATTRIBUTION = `Source: ${SOURCE_NAME}`;

/**
 * What to write beside a line so a reader sees at a glance what became of it.
 *
 * Nothing at all for a line whose arithmetic landed exactly, since a marker
 * beside every line would read as decoration rather than as the warning it is.
 */
export function lineMarker(line: ScaledIngredient): string {
  if (line.is_equipment) {
    return " [equipment]";
  }
  return line.scaling === "scaled" ? "" : ` [${line.scaling}]`;
}
...
export function toToolError(error: unknown): ToolResult {
  const known =
    error instanceof PequerecetasError
      ? error
